...
fun RouletteWheelForLocationBasedItem(
    ...
    // 다이얼로그 내에서 실제 크기를 기준으로 캔버스를 정사각형으로 맞춤
    androidx.compose.foundation.layout.BoxWithConstraints(
        modifier = Modifier
            .fillMaxWidth(0.9f) // 다이얼로그 너비의 90
            .aspectRatio(1f)
    ) {
        val canvasSizePx = with(LocalDensity.current) { maxWidth.toPx() }
        ...
        Canvas(
            ...
            if (items.isEmpty()) {
                ...
                items.forEachIndexed { index, tripItem ->
                    ...
                    val textX =
                        center.x + textRadius * cos(Math.toRadians(midAngle.toDouble())).toFloat()
                    val textY =
                        center.y + textRadius * sin(Math.toRadians(midAngle.toDouble())).toFloat()

                    // 텍스트를 해당 위치에 그림 (Android의 native Canvas 사용)
                    drawContext.canvas.nativeCanvas.apply {
                        drawText(
                            tripItem.title?.take(10) ?: "",
                            textX, // X좌표
                            textY, // Y좌표
                            Paint().apply {
